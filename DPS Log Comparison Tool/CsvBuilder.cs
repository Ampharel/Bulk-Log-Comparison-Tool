...
namespace DPS_Log_Comparison_Tool
{
    public class CsvBuilder()
    {
        ...
        public void CsvString(List<(ParsedEvtcLog, string)> logs, string OutputDirectory)
        {
            LoadFiles();
        ...
        public void ParseLogs(List<(ParsedEvtcLog,string)> logs, string OutputDirectory)
        {
            var FightLogs = logs.Select(x => x.Item1);
            var phasesPerLog = FightLogs.Select(x => x.FightData.GetPhases(x)).ToList();
            var allPhases = FightLogs.Select(x => x.FightData.GetPhases(x)).SelectMany(x => x).OrderBy(x => x.Start).ToList();
            ...
            var PlayerNames = FightLogs.Select(x => x.PlayerList).SelectMany(x => x).Select(x => (x.Account, x.Spec, x.Group)).OrderBy(x => x.Group).Distinct().ToList();
            ...
            foreach(var fileName in logs.Select(x => x.Item2))
            {
                sb.Append("," + fileName);
            }
            ...
            WriteMechanics(ref sb, FightLogs.ToList());

            sb.Append("\n");

            foreach (var phaseName in PhaseNames)
            {
                WritePhase(phaseName, ref sb, FightLogs.ToList(), PlayerNames);
            }
            if (sb.Length > 0)
            {
